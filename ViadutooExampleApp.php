 ...
 *
 * This is important since this program needs to accept input and
 * respond as quickly as possible.  It may take time for the sending
 * of data to complete and that shouldn't be allowed to get in the
 * way.
 *
...
$proxy = (new MessageProxy())
    ->setTransportInterface(
        (new CurlTransport()) // Recommended transport with cURL, supports Basic Auth and OAuth 1.0
            ...
            // ->setAuthZType(CurlTransport::AUTHZ_TYPE_BASICAUTH, 'username', 'password') // Optional authZ
    )
//    ->setTransportInterface(new PeclHttpTransport()) // Alternate transport with pecl_http, doesn't support authZ
    ...
    ->setStorageInterface(new SQLite3Storage(preg_replace('/\.php$/', '.sqlite3', __FILE__, 1))); // Simple storage with SQLite3, good enough for a demo
